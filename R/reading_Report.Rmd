...
reading <- read.xlsx("../data/reading_intro190227.xlsx", 1, 
                     startRow = 4, endRow = 84, 
                     colIndex = c(3:7, 9:82), 
...
reading$group <- class_roll$group %>%
  `[`(reading$id %>% match(class_roll$id))
...
leasure_hours_wd <- round((reading$Q1.1.h*60 + reading$Q1.1.m)/60, digits = 3)
leasure_hours_hd <- round((reading$Q1.2.h*60 + reading$Q1.2.m)/60, digits = 3)
reading_hours_wd <- round((reading$Q2.1.h*60 + reading$Q2.1.m)/60, digits = 3)
reading_hours_hd <- round((reading$Q2.2.h*60 + reading$Q2.2.m)/60, digits = 3)
where_1 <- factor(reading$Q3.1, levels = 1:6, labels = c("집", "직장(학교)", "출퇴근(통학)중", "도서관", "장소불문", "기타"))
where_2 <- factor(reading$Q3.2, levels = 1:6, labels = c("집", "직장(학교)", "출퇴근(통학)중", "도서관", "장소불문", "기타"))
reason_1 <- factor(reading$Q4.1, levels = 1:10, labels = c("업무", "학업, 취업", "교양, 인격", "지식, 정보", "즐거운 습관", "대화", "시간보내기", "위로와 평안", "실생활도움", "기타"))
reason_2 <- factor(reading$Q4.2, levels = 1:10, labels = c("업무", "학업, 취업", "교양, 인격", "지식, 정보", "즐거운 습관", "대화", "시간보내기", "위로와 평안", "실생활도움", "기타"))
...
obstacles_1 <- factor(obstacles)
obstacles_2 <- droplevels(obstacles)
volumes_p <- ifelse(reading$Q7.P <= 4, reading$Q7.P, "5권 이상")
volumes_p_f <- factor(volumes_p)
levels(volumes_p_f)
volumes_e <- ifelse(reading$Q7.E <= 4, reading$Q7.E, "5권 이상")
volumes_e_f <- factor(volumes_e)
levels(volumes_e_f)
levels(volumes_e_f) <- list("0" = "0", "1" = "1", "2" = "2", "3" = "3", "4" = "4", "5권 이상" = "5권 이상")
levels(volumes_e_f)
reader_p <- ifelse(reading$Q7.P >= 1, "Yes", "No")
reader_p_f <- factor(reader_p, levels = c("Yes", "No"), labels = c("1권 이상 읽는다", "1권도 읽지 않는다"))
reader_e <- ifelse(reading$Q7.E >= 1, "Yes", "No")
reader_e_f <- factor(reader_e, levels = c("Yes", "No"), labels = c("1권 이상 읽는다", "1권도 읽지 않는다"))
enough_f <- factor(reading$Q8, labels = c("매우 충분", "충분", "보통", "부족", "매우 부족"))
area_labels <- c("문학", "장르소설", "철학, 사상, 종교", "정치, 사회, 시사", "경제, 경영", "역사, 지리", "예술, 문화", "과학, 기술", "가정, 육아, 요리", "어학, 컴퓨터", "취미, 오락, 여행, 건강", "재테크, 부동산", "자기계발서", "기타")
area_p_1_f <- factor(reading$Q9.P.1, levels = 1:14, labels = area_labels)
area_p_2_f <- factor(reading$Q9.P.2, levels = 1:14, labels = area_labels)
area_p_3_f <- factor(reading$Q9.P.3, levels = 1:14, labels = area_labels)
area_e_1_f <- factor(reading$Q9.E.1, levels = 1:14, labels = area_labels)
area_e_2_f <- factor(reading$Q9.E.2, levels = 1:14, labels = area_labels)
area_e_3_f <- factor(reading$Q9.E.3, levels = 1:14, labels = area_labels)
info_labels <- c("서점/도서관", "신문/잡지", "TV/라디오", "인터넷", "가족/친구", "유명인/전문가", "각종기관추천", "베스트셀러", "드라마/영화원작", "블로그/SNS", "기타")
info_1_f <- factor(reading$Q10.1, levels = 1:11, labels = info_labels)
info_2_f <- factor(reading$Q10.2, levels = 1:11, labels = info_labels)
access_labels <- c("구입", "도서관", "주위사람", "선물", "대여점", "직장비치", "은행/관공서비치", "기타")
...
pander(tapply(leasure_hours_wd, reading$group, summary), digits = 3)
pander(summary(leasure_hours_wd), digits = 3, caption = "Overall")
```

### 휴일

```{r, leasure hrs holidays, echo = FALSE}
pander(tapply(leasure_hours_hd, reading$group, summary), digits = 3)
pander(summary(leasure_hours_hd), digits = 3, caption = "Overall")
```
...
pander(tapply(reading_hours_wd, reading$group, summary), digits = 3)
pander(summary(reading_hours_wd), digits = 3, caption = "Overall")
```

### 휴일

```{r, reading hrs holidays, echo = FALSE}
pander(tapply(reading_hours_hd, reading$group, summary), digits = 3)
pander(summary(reading_hours_hd), digits = 3, caption = "Overall")
```

## 독서 장소

```{r, reading location, echo = FALSE}
pander(table(reading$group, where_1), caption = "1순위")
pander(prop.table(table(where_1))*100, caption = "1순위(%)", digits = 3)
pander(table(reading$group, where_2), caption = "2순위")
pander(prop.table(table(where_2))*100, caption = "2순위(%)", digits = 3)
```

## 독서 이유

```{r, reason to read, echo = FALSE}
pander(table(reading$group, reason_1), caption = "1순위")
pander(prop.table(table(reason_1))*100, caption = "1순위(%)", digits = 3)
pander(table(reading$group, reason_2), caption = "2순위")
pander(prop.table(table(reason_2))*100, caption = "2순위(%)", digits = 3)
```
...
pander(table(reading$group, obstacles_1))
pander(prop.table(table(obstacles_1))*100, caption = "독서 어려운 이유(%)", digits = 3)
# pander(table(reading$group, obstacles_2))
```
...
pander(table(reading$group, volumes_p_f), caption = "종이책")
pander(prop.table(table(volumes_p_f))*100, caption = "종이책(%)", digits = 3)
pander(summary(reading$Q7.E), caption = "전자책", digits = 3)
pander(table(reading$group, volumes_e_f), caption = "전자책")
pander(prop.table(table(volumes_e_f))*100, caption = "전자책(%)", digits = 3)
```

## 독서율

```{r, proportion, echo = FALSE}
pander(table(reading$group, reader_p_f), caption = "종이책")
pander(prop.table(table(reader_p_f))*100, caption = "종이책(%)", digits = 3)
pander(table(reading$group, reader_e_f), caption = "전자책")
pander(prop.table(table(reader_e_f))*100, caption = "종이책(%)", digits = 3)
```

## 독서량은 충분한가?

```{r, enough, echo = FALSE}
pander(table(reading$group, enough_f), caption = "독서량 충분?")
pander(prop.table(table(enough_f))*100, caption = "독서량 충분?(%)", digits = 3)
```

## 독서 분야?

```{r, area, echo = FALSE}
pander(table(reading$group, area_p_1_f), caption = "종이책 1순위")
pander(prop.table(table(area_p_1_f))*100, caption = "종이책 1순위(%)", digits = 3)
pander(table(reading$group, area_p_2_f), caption = "종이책 2순위")
pander(prop.table(table(area_p_2_f))*100, caption = "종이책 2순위(%)", digits = 3)
pander(table(reading$group, area_p_3_f), caption = "종이책 3순위")
pander(prop.table(table(area_p_3_f))*100, caption = "종이책 3순위(%)", digits = 3)
pander(table(reading$group, area_p_1_f), caption = "전자책 1순위")
pander(prop.table(table(area_e_1_f))*100, caption = "전자책 1순위(%)", digits = 3)
pander(table(reading$group, area_e_2_f), caption = "전자책 2순위")
pander(prop.table(table(area_e_2_f))*100, caption = "전자책 2순위(%)", digits = 3)
pander(table(reading$group, area_e_3_f), caption = "전자책 3순위")
pander(prop.table(table(area_e_3_f))*100, caption = "전자책 3순위(%)", digits = 3)
```
...
area_p.df <- data.frame("제1순위" = area_p_1_f, "제2순위" = area_p_2_f, "제3순위" = area_p_3_f)
pander(area_p.df[most.read.ind, ], row.names = FALSE)
```
